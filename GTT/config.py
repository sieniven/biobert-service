    ...
    try:
        ...
        for section in sections:
            ...
            for option in options:
                full_option = f"{section}_{option}".upper()
                config_dict[option] = str(parser.get(section, option))
                config_dict[full_option] = str(parser.get(section, option))
    except Exception as e:
        logger.warning(f"Could not load configuration file due to exception {e}. "
...
__DEFAULT_CONFIG_DICT = populate_config_dict(default_config_path)
__LOCAL_CONFIG_DICT = populate_config_dict(config_path)
CONFIG_DICT = {**__DEFAULT_CONFIG_DICT, **__LOCAL_CONFIG_DICT}
...
class GTTConfigError(Exception):
    pass
...
def get_config(key, failure_ok=True):
    ...
    failure_ok : Optional[bool]
        If False and the configuration is missing, a GTTConfigError is
    ...
    if key in os.environ:
        ...
        if val is None and not failure_ok:
            ...
            raise GTTConfigError(msg)
        else:
            return val
    elif not failure_ok:
        raise GTTConfigError(err_msg)
    else:
        return None
